...
const footerLinks = [
  ...
  { label: "Solutions", key: "usecases", href: "#usecases" },
  { label: "Specifications", key: "specs", href: "#specs" },
  { label: "Contact", key: "inquiry", href: "#inquiry" },
];
      ...
      <div className="mx-auto max-w-7xl px-6 py-16">
        <div className="grid gap-10 md:grid-cols-4">
          <div className="md:col-span-2">
            ...
            <p className="mt-4 max-w-sm text-sm leading-relaxed text-white/40">{t("footer.description")}</p>
            <div className="mt-6 flex gap-3">
              {socials.map((s) => (
                <a key={s.name} href={s.href} className="flex h-9 w-9 items-center justify-center rounded-lg border border-white/10 text-white/40 transition-all hover:border-brand-500/40 hover:bg-brand-500/10 hover:text-brand-400" aria-label={s.name}>{s.icon}</a>
              ))}
            </div>
          </div>
          <div>
            ...
            <ul className="mt-4 space-y-3">
              {footerLinks.map((link) => (
                <li key={link.key}><a href={link.href} className="text-sm text-white/40 transition-colors hover:text-white">{link.label}</a></li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="mt-4 space-y-3 text-sm text-white/40">
              <li>{t("inquiry.emailContact")}</li>
              <li>{t("inquiry.phone")}</li>
              <li className="text-xs leading-relaxed">
                {lang === "zh" ? "汉方能源科技有限公司" : "Hanfang Energy Technology Co., Ltd."}<br />
        ...
        <div className="mt-14 flex flex-col items-center justify-between gap-4 border-t border-white/5 pt-8 text-xs text-white/30 md:flex-row">
          ...
          <div className="flex gap-6">
            <a href="#" className="transition-colors hover:text-white/50">{t("footer.privacy")}</a>
            <a href="#" className="transition-colors hover:text-white/50">{t("footer.terms")}</a>
          </div>
        </div>
      </div>
    </footer>
